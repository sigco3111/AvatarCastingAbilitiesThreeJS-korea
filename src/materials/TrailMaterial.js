    ...
    super({
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          ...
          vec3 np = vec3(along * uNoiseFrequency * 5.0 - u시간 * uFlowSpeed * 2.0,
                         across * 1.6,
                         u시간 * uFlowSpeed * 0.4);
          ...
          float pulse = exp(-pow((along - fract(u시간 * 0.35)) * 6.0, 2.0));
          float head = exp(-pow((along - uHead) * 9.0, 2.0));
          float sparks = pow(max(0.0, snoise(vec3(along * 60.0, vRandom * 12.0, u시간 * 6.0))), 6.0);

          // ---- length / dissolve ---------------------------------------
          float tailCut = smoothstep(uHead - uVisibleLength, uHead - uVisibleLength + 0.12, along);
          float burn = snoise01(vec3(along * 7.0, across * 2.5, u시간 * 0.6));
